...
### Added
- **xUnit Test Project**: Added `Irkalla.Kafka.Tests` covering configuration scanning and service resolution.
...
- **Observability (Metrics & Tracing)**:
  - Added OpenTelemetry distributed tracing using `ActivitySource("Irkalla.Kafka")` propagating `traceparent` headers.
  - Added metrics using `Meter("Irkalla.Kafka")` tracking message execution counts, durations, failures, DLQ publish, and retries.
- **Exponential Backoff**: Added retry options to `IrkallaKafkaOptions` containing `RetryDelay` supporting exponential backoff.
