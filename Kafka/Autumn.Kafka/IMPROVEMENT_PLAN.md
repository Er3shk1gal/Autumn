...
**Новая опция** в `AutumnKafkaOptions`:
...
**Проблема.** `AutumnKafkaOptions` — синглтон, но резолвится:
...
**Решение.** Добавить `AutumnKafkaOptions options` в primary constructor `BaseMessageHandler`, прокинуть из `CreateHandler`. Все `GetRequiredService<AutumnKafkaOptions>()` внутри хендлеров удалить. В `JsonMessageHandler` — обычный `using Microsoft.Extensions.DependencyInjection;` больше не нужен вовсе.

**Критерии приёмки:**
- [ ] `grep GetRequiredService<AutumnKafkaOptions>` по `MessageHandlers/` — пусто.

### 2.5. Запретить `EnableAutoCommit = true`

**Файл:** `Extensions/ServiceCollectionExtensions.cs` (`AddAutumnKafka`), `Configuration/AutumnKafkaOptions.cs`
...
if (options.EnableAutoCommit)
{
    throw new KafkaConfigurationException(
        "EnableAutoCommit is not supported: Autumn.Kafka commits offsets manually " +
...
**Критерии приёмки:**
- [ ] Опция удалена ИЛИ `AddAutumnKafka` бросает исключение при `true`.
...
- `ActivitySource("Autumn.Kafka")`: activity на обработку сообщения (`autumn.kafka.consume`) с тегами `messaging.system=kafka`, `messaging.destination`, `method`, и на produce ответа/DLQ.
...
- Счётчики через `System.Diagnostics.Metrics.Meter("Autumn.Kafka")`: `messages_processed`, `messages_failed`, `messages_dlq`, `retry_attempts`, histogram `processing_duration`.
...
Проект `Autumn.Kafka.Tests` (xUnit).
