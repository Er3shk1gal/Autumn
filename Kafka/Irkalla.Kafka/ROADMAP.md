# Irkalla.Kafka — дорожная карта до 1.0
...
**Остаётся (config-DX):**
- `AddIrkallaKafka(IConfiguration)` + биндинг appsettings, `IOptions`, ранняя агрегированная валидация, дефолты под минимальный hello-world.
- Producer-only: типизированный `IKafkaProducer.SendAsync<T>(topic, method, payload, key?, correlationId?, messageId?)` + `AddIrkallaKafkaProducer(...)` без `GroupId`/скана/консюмеров; снять жёсткое требование `GroupId`, когда консюмеров нет.
...
// }
// builder.Services.AddIrkallaKafkaRpcClient();   // после AddIrkallaKafka(...)
```
...
6. **DI** — `AddIrkallaKafkaRpcClient` + overload для чистого клиента без `[KafkaService]`.
...
- **Трейсинг:** `ActivitySource("Irkalla.Kafka")` — activity на обработку и на produce; проброс `traceparent` (частично есть). Сквозной trace producer→consumer→response через OpenTelemetry без обязательных зависимостей.
- **Метрики:** `Meter("Irkalla.Kafka")` — `messages_processed/failed/dlq`, `retry_attempts`, `processing_duration`; для RPC — `rpc_calls/timeouts/orphans`, `rpc_call_duration`, gauge `rpc_reply_lag`. Опциональный `IHealthCheck`.
